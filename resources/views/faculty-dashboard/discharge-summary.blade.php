    ...
    <form id="patients_medicines" name="patients_medicines" method="post" autocomplete="on">
      <div class="row">
            ...
            <div class="col-md-3 getdata">
                <button type="button" class="btn" id="getdata" onclick="get_patient_history();">Get Data</button>
            </div>
        </div>
    </form>
                ...
                <div class="col-md-9">
                    <div class="activityhead">
                        <h5 class="mb-0 ml-4">Probationer List</h5>
                            ...
                            <?php
                                $get_discharge_summary = DB::table('in_patients')
        ...
        {{-- <div class="listdetails1">
            <div class="table-responsive">
                <table class="table" id="probationerslist1">
                    <thead>
                        <tr>
                        <th>S.No</th>
                        <th>Patient Id</th>
                    ...
                    <tbody>
                            @foreach ($get_discharge_summary as $key => $get_discharge_summarys)
                            <?php
                                    // $key++;
                            ?>
                            <tr>
                                <td>{{ $key }}  </td>
                                <td>{{ $get_discharge_summarys->RollNumber }}</td>
                                <td>{{ $get_discharge_summarys->Name }}</td>
                                <td>{{ $get_discharge_summarys->admitted_date }}</td>
                                <td>{{ $get_discharge_summarys->discharge_date }}</td>
                                <td></td>
                            </tr>
                                @endforeach
                    </tbody>
          ...
          $.ajax({
            ...
            data:{
                ...
                if(data == '')
                {
                    ...
                    //$('#probationerslist thead').append('<tr><th>No Discharge Summary Found for this Probationer</th></tr>');
